...
def print_statistics(instance, days, dataset, disp, method='greedy', dir='greedy', wip = False, seed=0):
    ...
    myfile = os.path.join(
        os.environ.get('PYSCFABSIM_DEBUG_DIR', 'debug_data'),
        f'debug_data_{method}_{days}days_{dataset}_{disp}.pkl')
    try:
        os.makedirs(os.path.dirname(myfile), exist_ok=True)
        with open(myfile, 'wb') as f:
            pickle.dump(debug_data, f)
    except Exception as e:
        print("Konnte Datei nicht speichern:", myfile, e)
    ...
    if acts:
        ...
        # No lot finished its route yet - happens on very short runs.
        print('no completed lots')
    print('---------------')
