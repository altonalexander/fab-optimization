    ...
    for t in ['lots', 'machines']:
        ...
        for ds in ['HVLM', 'LVHM']:
            ...
            if not path.isfile(file_name):
                # Reference baselines from the SMT2020 paper are not shipped with
                # this repo. Without them the 'old' columns fall back to 0, so the
                # deltas are NOT meaningful comparisons.
                sys.stderr.write(
                    f'WARNING: missing reference file {file_name}; '
                    f'{t} baseline columns for {ds} will read 0 and deltas are meaningless\n')
                continue
    ...
    for d in dirs:
        ...
        for s in subs:
            if path.isfile(d + '/' + s) and 'seed0' in s and s.endswith('.json'):
                # Only aggregate the seeds that were actually run, so a partial
                # sweep still reports instead of dying on a missing file.
                file_locs = [d + '/' + s.replace('seed0_', f'seed{r}_') for r in range(runs)]
                file_locs = [f for f in file_locs if path.isfile(f)]
                if not file_locs:
                    continue
    ...
    for name in results.keys():
        ...
        for k, v in sorted(r['machines'].items(), key=lambda k: k[0]):
            machines[k][name] = dict(
                ...
                # Machine-level waiting_time is currently not emitted by the
                # simulator (commented out in stats.py), so default it to 0.
                waiting_time=round(v["waiting_time"][0], 2) if "waiting_time" in v else 0,
            )
