...
**2026-09-17 06:55Z lead** — **crit v3 on FAIR warm-ups: its scale-3 win does not hold.** ship/d · share: s0 qtfw **47.7 · 7.0%** vs v3 46.9 · 21.6%; s2 qtfw 47.5 · 14.5% vs v3 **49.9 · 14.2%** (tie). The earlier v3 win was an artifact of resuming from qt warm-ups. **No scheduler version (v1–v4, hold/no-hold, u300/u600/load) beats tuned qtfw on fair checkpoints.** RELEASE all lead slots.

**Grid (all lanes done, GRID_TABLE_auto.txt, 5 seeds, 60 d; good/d · share):** 8: qt 56.2 · 2.0%, qtfw 57.3 · 0.4%; 5: qt 54.2 · 7.6%, qtfw 55.0 · 2.4%; 4: qt 47.1 · 18.8% BREAKS, **qtfw(Wqtfw) 52.0 · 5.5% viable**; 3: qt 37.4 · 34.4%, qtfw(Wqtfw) 49.4 · 12.2% BREAKS (one seed 15.8%; 90-d cells viable at 12.0%); 2: qt 26.3 · 54%, qtfw 43.9 · 23.5%; 1: qt 14.0 · 75%, qtfw 29.5 · 45%. fifo/cr break at ≤5 (fifo also at 8 by share). **Breakdown map: plain qt fails at scale 4; the realistic under-min rule fails at 3–2 — the critical range.**
Next (lead): crit v5 = conservative hybrid — qtfw everywhere, CP-SAT plan overrides only where it saves a windowed lot qtfw would blow; A/B on fair warm-ups scale 3 and 2, seeds 0,2.
...
**2026-09-17 15:30Z lead** — **v5 grid (60 d, 14/15 cells, paired vs qtfwK6s8Wqtfw on the same qtfw warm-ups): v5 does NOT beat qtfw.** Mean deltas v5−qtfw: scale 4 (n=4) +0.5 good/d, −0.5 pt share (lower on 3/4); scale 3 (n=5) −0.5 good/d, **+0.8 pt** (lower on 2/5); scale 2 (n=5) +0.7 good/d, +0.2 pt (lower on 2/5). All within seed spread; v5 does not move scale 3 into the viable band. The 15-d 3-of-4 signal did not hold. `bench/results/grid/GRID_PAIRED_v5_w60_partial.txt`. v5 90-d scale-3 cells still to run (chained).
**Status of the optimiser question:** across v1–v5 (tool/family models, holds on/off, priced under-min fixed/load, hybrid) no CP-SAT variant measurably outperforms the tuned realistic rule (qtfw) on fair warm-ups in the critical range.
Proposed next (lead): a fair headroom bound from the qtfw warm-ups at scale 3 (batch_min=1; extra furnace capacity) to test whether the remaining 10–15% scrap is decision-limited (an optimiser could win) or capacity-limited (nothing can). Coordinator: fold v5 into GRID_SUMMARY + lab note plainly when the 90-d cells land.

**2026-09-17 16:00Z lead** — **Fair headroom bound** (qtfw + batch_min=1, from qtfw warm-ups, 15 d, `bench/results/bound_fair/`), share qtfw → bound: scale 3 s0 7.0% → **2.4%**, s2 14.5% → **6.3%** (ship/d ≈ level); scale 2 s0 31.2% → 22.7%, s2 14.7% → 13.9%. **Scale 3 is decision-limited (~55–65% of qtfw scrap recoverable by batching choices); scale 2 is mostly capacity-limited.** So the optimiser failure at scale 3 is a design failure, not an impossibility; the bound is a relaxation (free underfilling), so the true reachable gap is smaller. Awaiting user direction before further optimiser design; v5 90-d cells still chained.
